...
def topology():
    ...
    h2 = net.addHost('h2',ip='172.20.0.1/16', defaultRoute='172.20.0.3/16')
    h3 = net.addHost('h3',ip='140.114.0.1/24', defaultRoute='140.114.0.2/24')
    ...
    net.start()
    config(routers, h2, h3, DHCPServer)
    runBGP(routers)
    runDHCP(net) # if your dhcpd.conf is done, uncomment this line 
    addNAT(routers)
    # Comment this line if you don't need to debug
    CLI(net)
    killDHCP(net) # Kill DHCP server process, don't leave dhcp process on your computer 
    killBGP()
    net.stop()

def config(routers, h2, h3, DHCPServer):
    DHCPServer.cmd('ifconfig DHCPServer-eth0 192.168.1.4/24')
    h2.cmd('route add default gw 172.20.0.3')
    h3.cmd('route add default gw 140.114.0.2')
    routers['r1'].cmd('ifconfig r1-eth0 10.0.1.2/24')
    routers['r1'].cmd('ifconfig r1-eth1 192.168.1.254/24')
    routers['r1'].cmd('ifconfig r1-eth2 172.20.0.3/16')
...
def addNAT(routers):
    routers['r1'].cmd('iptables -t nat -A POSTROUTING -s 192.168.1.0/24 -d 0.0.0.0/0 -o r1-eth0 -j SNAT --to-source 140.113.0.40')
    routers['r1'].cmd('iptables -t nat -A POSTROUTING -s 172.20.0.0/16 -d 0.0.0.0/0 -o r1-eth0 -j SNAT --to-source 140.113.0.50')
